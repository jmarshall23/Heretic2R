...
{
	g_gl.curColor[0] = r / 255.0f;
	g_gl.curColor[1] = g / 255.0f;
	g_gl.curColor[2] = b / 255.0f;
	g_gl.curColor[3] = 1.0f;
}
...
extern "C" void APIENTRY glActiveTexture(GLenum texture)
{
	...
	const GLuint unit = (GLuint)(texture - GL_TEXTURE0);
	if (unit >= QD3D12_MaxTextureUnits)
	{
		g_gl.lastError = GL_INVALID_ENUM;
		return;
	}

	g_gl.activeTextureUnit = unit;
}
...
extern "C" void APIENTRY glClientActiveTexture(GLenum texture)
{
	...
	g_gl.clientActiveTextureUnit = unit;
}

extern "C" void APIENTRY glMultiTexCoord2f(GLenum texture, GLfloat s, GLfloat t)
{
	if (texture < GL_TEXTURE0)
	{
		g_gl.lastError = GL_INVALID_ENUM;
...
extern "C" void APIENTRY glTexEnvi(GLenum target, GLenum pname, GLint param)
{
	...
	if (pname == GL_TEXTURE_ENV_MODE)
	{
		if (g_gl.activeTextureUnit < QD3D12_MaxTextureUnits)
			g_gl.texEnvMode[g_gl.activeTextureUnit] = (GLenum)param;
		return;
	}
...
{
	// Enough to satisfy legacy fixed-function sphere-map setup calls.
	// Your shader path does not currently implement fixed-function texgen,
	// so keep it as a compatibility no-op instead of erroring.
	(void)coord;
	(void)pname;
	(void)param;
}
...
{
	// Safe compatibility implementation.
	// Most Quake-era upload/readback code only cares that this symbol exists.
	switch (pname)
	{
